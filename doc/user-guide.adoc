...
For Gradle, use `bootRunLocal`. It generates the local S3A JCEKS file and runs the app with `spring.profiles.active=local`:
...
For IntelliJ IDEA, run the JCEKS preparation task once:

[source,bash]
----
GRADLE_USER_HOME=/data/.gradle ./gradlew :example:spring:prepareSpringLocalS3Jceks
----

Then run `org.openprojectx.bigdata.test.example.spring.BigDataTestExampleApplicationKt` with active profile `local`. Because the starter dependency is `runtimeOnly` for non-CI builds, the same config-driven container startup works from the IDE.
