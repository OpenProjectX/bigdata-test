...
Iceberg metadata and Parquet data files exist in S3. Run it with:
...
The table also accepts `catalogBackend`, `uri`, `jdbcDriver`, `jdbcUser`, and
`jdbcPassword`. Programmatic users call
