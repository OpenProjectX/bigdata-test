...
The Spring example is a normal Spring Boot web application. Its default configuration keeps the S3A REST API disabled, so CI builds do not start containers and do not include the bigdata-test Spring starter on the application runtime classpath.
...
The local developer profile is fully configuration driven. The app source does not call `BigDataTestKit`; the custom `bootRunLocal` task adds `bigdata-test-spring-boot-starter` only to that run, generates a local S3A JCEKS file, starts Kerberos, HDFS, and LocalStack S3 from `application-local.yaml`, and enables the REST API.
...
The generated JCEKS file is written to:

[source,text]
----
example/spring/build/spring-local/s3a.jceks
----
...
* `fs.s3a.aws.credentials.provider=org.apache.hadoop.fs.s3a.SimpleAWSCredentialsProvider`
* `hadoop.security.credential.provider.path=jceks://file/${user.dir}/build/spring-local/s3a.jceks`
