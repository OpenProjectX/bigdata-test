...
* Gravitino Iceberg REST catalog server
...
|`icebergRestCatalog`
|Starts a Gravitino Iceberg REST catalog server.
...
icebergRestCatalog = "ghcr.io/openprojectx/gravitino-iceberg-rest:0.1.1"
trino = "ghcr.io/openprojectx/dockerhub/trinodb/trino:483"
----

=== Iceberg REST Catalog

The catalog listens on container port `9001` and is ready when
`/iceberg/v1/config` returns HTTP 200. The client URI is exposed through
`iceberg.rest.uri`, `spark.sql.catalog.rest.uri`, and
`bigdata.test.iceberg-rest-catalog.uri`.

The default metadata store is in-memory SQLite and the default warehouse is
container-local, so the minimal configuration has no storage dependency:
...
For S3-backed tables, enable S3 and configure the warehouse and file IO. The
framework supplies the catalog container with the same-instance internal S3
endpoint (`http://s3:4566`), credentials, region, and path-style setting. The
bucket must exist before a table is created.
...
Floci `1.7.0` also provides STS. Enable Gravitino's `s3-token` credential
provider to vend temporary, path-scoped credentials through Iceberg REST:
...
The framework supplies Gravitino's base S3 access key and secret. Gravitino
uses those only to call STS; they are not returned as the temporary client
identity. A REST client requests vending with
...
expiration. The same fields are available through
`IcebergRestCatalogOptions` for programmatic configuration.

The configured `s3TokenServiceEndpoint` is container-side and therefore uses
the `s3` network alias. Gravitino also returns its configured S3 endpoint in
the credential response. With random host ports, a host-side SDK should use
the S3 endpoint exposed by `BigDataService.S3` together with the vended
credentials. Directly handing the returned Docker endpoint `http://s3:4566`
to a host process will not work unless the host deliberately provides matching
DNS and port routing.
...
The table also accepts `catalogBackend`, `uri`, `jdbcDriver`, `jdbcUser`,
`jdbcPassword`, `credentialProviders`, `s3RoleArn`, `s3ExternalId`, and
`s3TokenServiceEndpoint`. Programmatic users call
`withIcebergRestCatalog(IcebergRestCatalogOptions(...))`. Advanced image-specific
settings can be passed through `[containers.icebergRestCatalog.env]` or the
general container customization API.
...
|`http`, `https`
|`9001`, `443`
|`iceberg.rest.uri`, `spark.sql.catalog.rest.uri`, `bigdata.test.iceberg-rest-catalog.uri`, `bigdata.test.iceberg-rest-catalog.internal-uri`
|===
