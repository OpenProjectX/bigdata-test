...
* Apache Polaris Iceberg REST catalog server
...
Trino is a built-in managed service. It starts after HMS, uses the stable `hive-metastore:9083` network address, and exposes `trino.url`, `trino.jdbc.url`, and `spring.bigdata.test.trino.url` to host-side clients. Enabling Trino automatically enables open-source HMS unless `clouderaHms` is selected.
...
[images]
trino = "ghcr.io/openprojectx/dockerhub/trinodb/trino:483"

[ports]
trino = 0 # random host port; container port remains 8080
...
When S3 is enabled in the same instance, the generated Hive catalog automatically enables Trino native S3 support with `http://s3:4566`, test credentials, `us-east-1`, and path-style access. Entries in `[trino.catalogProperties]` are applied last and can override generated values or add HDFS/GCS/image-specific connector settings.

Programmatic configuration uses `withTrino(TrinoOptions(...))`. JUnit can use `@BigDataTest(trino = true)`. The Gradle DSL uses `services.trino.set(true)` and `trino { ... }`; Spring Boot uses `bigdata.test.trino.enabled=true`. The executable `TrinoBigDataTestExample` sends `SHOW SCHEMAS FROM hive` through Trino's statement API and verifies the managed HMS catalog.

=== User-Provided Containers

Applications can attach any user-created `GenericContainer` to a test-kit service network. The core API does not inspect the image or contain product-specific integration logic. The user configures the container with the addresses returned by `containerEndpoint(...)`.

`attachContainer(...)` only joins the selected network. The caller owns the container lifecycle:

[source,kotlin]
----
val worker = GenericContainer<Nothing>(DockerImageName.parse("my-company/worker:latest"))
kit.attachContainer(worker, aliases = arrayOf("worker"))
worker.start()
try {
    // test
} finally {
...
`startContainer(...)` joins and starts the container after the kit services are ready. The kit then stops it before closing the Docker network:
...
kit.startContainer(worker, aliases = arrayOf("worker"))
----

The equivalent Java API retains the concrete container type:

[source,java]
----
GenericContainer<?> worker = new GenericContainer<>(DockerImageName.parse("my-company/worker:latest"));
BigDataContainerEndpoint hms = kit.containerEndpoint(BigDataService.HIVE_METASTORE);
String metastoreUri = hms.uri("thrift", "thrift");
worker.withEnv("HMS_URI", metastoreUri);
kit.startContainer(worker, "default", "worker");
----

The container must not be running when it is first attached. A container can join one service-instance network only. For a named instance, use the same instance for both operations:

[source,kotlin]
----
val hms = kit.containerEndpoint(BigDataService.HIVE_METASTORE, "analytics")
kit.startContainer(worker, "analytics", "worker")
----

Each `BigDataContainerEndpoint` contains the stable Docker alias and container ports, not host-mapped ports. For example, HMS is `thrift://hive-metastore:9083`, S3 is `http://s3:4566`, and HDFS uses the configured NameNode port at host `hdfs`. Use `endpoint(...)` instead when code runs in the host JVM.

User-provided containers do not automatically receive generated keytabs, truststores, or XML configuration. Copy the required material into the container and configure that product explicitly. Network attachment is incompatible with a service customized to Docker host network mode because that service is no longer on the instance bridge network.
...
Each named instance is a complete, isolated service stack. Configuration from the `default` instance is not inherited by named instances. Declare every service that must communicate inside the same `[instances.<name>.services]` table.

For example, this creates an `analytics` HMS that can use the `analytics` HDFS, S3, GCS emulator, and Kerberos KDC:
...
The containers in that instance share one Docker network and use stable internal aliases:
...
|Hive Metastore
|GCS emulator
|`http://fake-gcs:<service-port>`

|HDFS, Hive Metastore, Kafka, or Kafka UI
|Kerberos KDC
|`kerby-kdc:88`

|Schema Registry or Kafka UI
|Kafka
|The `kafka` alias and the security-specific internal listener
|===

Aliases can be reused by every instance because each instance has a different Docker network. The framework generates and transfers Kerberos configuration and keytabs separately for each instance.

Default-instance services are not visible from a named instance. The following configuration does _not_ connect the `analytics` HMS to the default S3 emulator:
...
Enable `instances.analytics.services.s3` when the analytics HMS must access S3. Direct container-to-container wiring across instances is not supported because the stacks are intentionally on separate networks. Host-side extensions may explicitly combine instances when their API provides separate instance selectors. For example, `S3JceksExtension` supports `hdfsInstance` and `s3Instance` because it connects through the endpoints exposed to the test JVM.

Host applications and tests should use the mapped endpoint for the required instance rather than an internal Docker alias:

[source,kotlin]
----
val analyticsHms = kit.endpoint(BigDataService.HIVE_METASTORE, "analytics")
val analyticsS3 = kit.endpoint(BigDataService.S3, "analytics")

val metastoreUri = analyticsHms.property("hive.metastore.uris")
val s3Endpoint = analyticsS3.property("aws.endpoint-url.s3")
----

==== Generated Files

Default generated files are isolated per instance. Each instance receives separate temporary directories for:

* `hdfs-site.xml`
* `hive-site.xml` and `metastore-site.xml`
* Kerberos `krb5.conf` and keytabs
* generated TLS certificates, keystores, and truststores

Retrieve a generated path from the corresponding named endpoint:

[source,kotlin]
----
val analyticsHiveSite = kit.endpoint(BigDataService.HIVE_METASTORE, "analytics")
    .property("bigdata.test.hive-metastore.hive-site")
val analyticsHdfsSite = kit.endpoint(BigDataService.HDFS, "analytics")
    .property("bigdata.test.hdfs.hdfs-site")
----

Explicit local output paths are exact paths. The framework does not append the instance name and does not reject duplicate paths. Configure distinct paths when multiple instances generate files:

[source,toml]
----
[instances.analytics.hdfs]
localHdfsSitePath = "build/bigdata-test/analytics/hdfs-site.xml"

[instances.analytics.hiveMetastore]
localHiveSitePath = "build/bigdata-test/analytics/hive-site.xml"
localMetastoreSitePath = "build/bigdata-test/analytics/metastore-site.xml"

[instances.archive.hdfs]
localHdfsSitePath = "build/bigdata-test/archive/hdfs-site.xml"

[instances.archive.hiveMetastore]
localHiveSitePath = "build/bigdata-test/archive/hive-site.xml"
localMetastoreSitePath = "build/bigdata-test/archive/metastore-site.xml"
----

The same rule applies to Kerberos `materialDirectory`, explicit local Kerberos copy paths, TLS `trustStorePath`, and extension copy destinations. Reusing read-only input files such as `caCertPath` and `caKeyPath` is valid and is the expected way to make multiple instances trust the same stable CA. Generated output files should remain instance-specific.
...
|`icebergRestCatalog`
|Starts an Apache Polaris Iceberg REST catalog server and provisions a catalog.

|`trino`
|Starts Trino with a generated Hive catalog connected to same-instance HMS.
|===
...
s3 = "ghcr.io/openprojectx/dockerhub/floci/floci:1.7.0"
fakeGcs = "ghcr.io/openprojectx/dockerhub/floci/floci-gcp:0.5.0"
icebergRestCatalog = "ghcr.io/openprojectx/dockerhub/apache/polaris:1.7.0"
trino = "ghcr.io/openprojectx/dockerhub/trinodb/trino:483"
----

=== Iceberg REST Catalog

The built-in Iceberg REST service uses Apache Polaris. Its catalog API listens
on container port `8181`; management and health use `8182`. The framework waits
for `/q/health`, obtains a root OAuth token, and creates the configured catalog
before publishing the endpoint.

The default catalog is named `bigdata_test` and uses the container-local
`file:///tmp/iceberg/warehouse` location, so the minimal configuration has no
...
For S3-backed tables, enable S3 and configure the catalog name, warehouse, and
role. The framework gives Polaris both the mapped host endpoint returned to
clients and the same-instance `http://s3:4566` endpoint used by Polaris itself.
Region, path-style access, and STS are configured automatically. The bucket
must exist before a table is created.
...
s3RoleArn = "arn:aws:iam::000000000000:role/iceberg-rest"
# s3ExternalId = "optional-cross-account-id"
----

Floci `1.7.0` provides STS. Polaris uses the configured role to vend temporary,
path-scoped credentials. A REST client requests vending with
`X-Iceberg-Access-Delegation: vended-credentials`. The response contains
`s3.access-key-id`, `s3.secret-access-key`, `s3.session-token`, and the session
expiration. With random ports, host clients receive the mapped S3 endpoint;
Polaris retains the Docker-network endpoint for its own metadata writes.

Create the warehouse bucket from the extensions configuration before the test
method creates tables:

[source,toml]
----
[[s3Buckets]]
id = "iceberg-rest-warehouse"
bucket = "iceberg"
----

`example/junit/IcebergRestCatalogExampleTest` is an executable end-to-end
example. It creates an Iceberg table through the REST API, loads the table
again, and lists S3 objects to verify that Iceberg metadata was written below
`s3://iceberg/warehouse/<namespace>/<table>/metadata`.

`example/spark/SparkIcebergRestS3Example` is the full data-path smoke test. It
configures Spark's Iceberg `SparkCatalog` with `type=rest`, inserts rows into an
Iceberg table, reads the rows back through Spark, and verifies that both the
Iceberg metadata and Parquet data files exist in S3. It then requests vended
credentials, verifies the STS caller is the assumed role, and uses that session
to list the table's Parquet objects. Run it with:

[source,bash]
----
GRADLE_USER_HOME=/data/.gradle ./gradlew :example:spark:sparkIcebergRestS3Test
----

The `[icebergRestCatalog]` table accepts `catalogName`, `warehouse`, `realm`,
`clientId`, `clientSecret`, `scope`, `s3RoleArn`, `s3ExternalId`, and
`startupTimeoutSeconds`. Programmatic users call
`withIcebergRestCatalog(IcebergRestCatalogOptions(...))`. The endpoint publishes
`iceberg.rest.uri`, `warehouse`, `credential`, `scope`, `oauth2-server-uri`,
`token`, and `realm` properties, plus corresponding `spark.sql.catalog.rest.*`
properties. Advanced image-specific settings can be passed through
`[containers.icebergRestCatalog.env]` or the general container customization API.

==== Migration from Gravitino

The built-in implementation changed from Gravitino to Polaris. Remove the old
`catalogBackend`, `uri`, `jdbcDriver`, `jdbcUser`, `jdbcPassword`, `ioImpl`,
`credentialProviders`, and `s3TokenServiceEndpoint` fields. Polaris catalog
provisioning replaces them. REST routes are authenticated and catalog-scoped;
clients should consume the endpoint properties instead of constructing the old
`/iceberg` URI.
...
Available service names in container TOML are `kerberos`, `hdfs`, `hiveMetastore`, `hms`, `kafka`, `schemaRegistry`, `kafkaUi`, `s3`, `fakeGcs`, `icebergRestCatalog`, and `trino`.
...
Every service uses a Testcontainers wait strategy to detect process startup. Kafka additionally runs `kafka-broker-api-versions.sh` against an internal broker listener before publishing its endpoint. This mandatory protocol probe does not depend on a particular Kafka log message, and it supports the framework's plaintext, TLS, and Kerberos configurations.

Configure the timeout used by each Kafka startup gate (the port wait and the protocol probe) with TOML:
...
The equivalent programmatic option is `KafkaOptions(startupTimeoutSeconds = 180)`. Spring Boot exposes `bigdata.test.kafka.startup-timeout-seconds`, and the Gradle plugin DSL exposes `kafka.startupTimeoutSeconds`.

Optional health checks run after a service starts and after its endpoint is registered. Use these for an additional service-specific smoke check.
...
|Kafka
|Runs `kafka-topics.sh --list` against the in-container broker endpoint. This is additional to Kafka's mandatory `kafka-broker-api-versions.sh` startup probe.
...
CLI probes validate the container's own environment. They do not prove that an advertised listener, mapped host port, host DNS, or client security configuration is usable. Use a Kafka `AdminClient`, producer, or consumer smoke test from the test JVM for that boundary. The same distinction applies to Windows HDFS clients: an in-container `hdfs dfs` command cannot prove host network reachability.
...
Available TLS port fields are `hdfsWebTls`, `schemaRegistryTls`, `kafkaUiTls`, `s3Tls`, `fakeGcsTls`, and `icebergRestCatalogTls`.
...
|Iceberg REST catalog HTTP
|`icebergRestCatalog`
|`icebergRestCatalogPort`
|`8181` (`8182` is the dynamically mapped Polaris management/health port)

|Iceberg REST catalog TLS gateway
...
|`8181`, `8182`, `443`
|`iceberg.rest.uri`, `iceberg.rest.warehouse`, `iceberg.rest.credential`, `iceberg.rest.scope`, `iceberg.rest.oauth2-server-uri`, `iceberg.rest.token`, `iceberg.rest.realm`, `spark.sql.catalog.rest.*`, `bigdata.test.iceberg-rest-catalog.*`
|===
...
=== Trino SQL Preparation Extension

The `trino-sql-prep` extension connects to the managed Trino service through its JDBC endpoint after all containers have started. It executes statements and classpath SQL scripts in declaration order. This makes it suitable for work that should follow Spark preparation, such as creating Trino views over Iceberg tables that Spark registered in the shared HMS.
...
statements = [
  "CREATE TABLE demo.events USING iceberg AS SELECT 1 AS id",
]

[[extensions]]
type = "trino-sql-prep"
id = "create-trino-view"
...
statements = [
  "CREATE OR REPLACE VIEW events_view AS SELECT * FROM events",
]
scripts = [
  "classpath:sql/create_trino_views.sql",
]
----

The matching Trino service catalog must point at the same HMS and object store used by Spark. The built-in Trino service wires its catalog to the selected HMS and adds native S3 settings when S3 is enabled. Extension declaration order matters: the Spark extension must appear before the Trino extension when Trino SQL depends on Spark-created metadata.

Programmatic configuration supports values generated for a test run:
...
extensions.trinoSqlPreparation {
    ...
    catalog = "iceberg"
    schema = "demo_$suffix"
    statement("CREATE VIEW events_view AS SELECT * FROM events")
}
----

[source,java]
----
extensions.trinoSqlPreparation(trino -> {
    trino.setId("create-trino-view");
    trino.setCatalog("iceberg");
    trino.setSchema("demo");
    trino.statement("CREATE VIEW events_view AS SELECT * FROM events");
});
----

The extension publishes `<id>.engine = trino-jdbc` and `<id>.executed-statements`. `TrinoSqlClient` is also public for assertions and ad hoc SQL; construct it from `kit.endpoint(BigDataService.TRINO).property("trino.jdbc.url")` and close it after use.

Direct JUnit users of the thin `extensions` artifact must provide the JDBC driver on the test runtime, for example `testRuntimeOnly("io.trino:trino-jdbc:483")`. The Gradle plugin detects `trino-sql-prep` and supplies the driver on its isolated extension runtime; its shaded runtime artifact already contains the driver.

The executable example creates an HMS-backed Iceberg table and data on S3 with Spark, creates a Trino view, then queries that view through JDBC:

[source,shell]
----
GRADLE_USER_HOME=/data/.gradle ./gradlew :example:spark:sparkTrinoIcebergViewTest
----
...
The Gradle plugin supports two isolated Spark runtime lines:

[cols="1,1,1,2",options="header"]
|===
|Spark |Scala |Jackson |Shaded runtime

|3.5.x
|2.12
|2.15.2
|`extensions:<version>:runtime`

|4.1.x
|2.13
|2.20.x
|`extensions-spark-4:<version>:runtime`
|===

Select Spark 4.1.1 with:

[source,kotlin]
----
bigDataTest.extensionRuntime.sparkVersion.set("4.1.1")
----

`scalaBinaryVersion` defaults to `2.12` for Spark 3 and `2.13` for Spark 4. The plugin rejects unsupported Spark lines and mismatched Spark/Scala combinations during configuration.
...
By default the plugin uses a shaded runtime selected from `sparkVersion`:
`extensions:<version>:runtime` for Spark 3.5 or
`extensions-spark-4:<version>:runtime` for Spark 4.1. This keeps the two Spark
dependency graphs isolated from the application and from each other.
...
* `trinoSqlPreparation` or `type = "trino-sql-prep"` enables the Trino JDBC driver.
...
bigDataTest {
    extensionRuntime {
        ...
        hadoopVersion.set("3.4.2")
        sparkVersion.set("4.1.1")
        scalaBinaryVersion.set("2.13") // optional; inferred from sparkVersion
        confluentVersion.set("8.2.1")
        avroVersion.set("1.12.1")
        lz4Version.set("1.10.1")
        awsSdkVersion.set("2.41.5")
        trinoVersion.set("483")
    }
}
----
...
[source,properties]
----
org.gradle.jvmargs=-Xmx4096M -Dfile.encoding=UTF-8 --add-exports=java.base/sun.nio.ch=ALL-UNNAMED --add-opens=java.base/java.lang=ALL-UNNAMED --add-opens=java.base/java.lang.invoke=ALL-UNNAMED --add-opens=java.base/java.lang.reflect=ALL-UNNAMED --add-opens=java.base/java.io=ALL-UNNAMED --add-opens=java.base/java.net=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED --add-opens=java.base/java.util=ALL-UNNAMED --add-opens=java.base/java.util.concurrent=ALL-UNNAMED --add-opens=java.base/java.util.concurrent.atomic=ALL-UNNAMED --add-opens=java.base/sun.nio.ch=ALL-UNNAMED --add-opens=java.base/sun.nio.cs=ALL-UNNAMED --add-opens=java.base/sun.security.action=ALL-UNNAMED --add-opens=java.base/sun.util.calendar=ALL-UNNAMED --add-opens=java.security.jgss/sun.security.krb5=ALL-UNNAMED
----
...
Errors naming an inaccessible JDK class or saying that `java.base` does not
`open` or `export` a package indicate that the daemon was started without the
required module access. `spark.executor.extraJavaOptions` does not fix the
driver in this case because `spark-sql-prep` creates its Spark driver inside
the Gradle daemon.
...
bigDataTest {
    extensionRuntime {
        ...
        includeTrinoJdbc.set(true)
    }
}
----

The `extensions` and `extensions-spark-4` modules publish shaded runtime artifacts with classifier `runtime`. This is the default plugin mode:
...
With `useShadedArtifact = true`, the plugin resolves one of these artifacts
into `bigDataTestExtensionRuntime` and skips adding separate Hadoop, Spark,
Iceberg, Kafka, and Avro extension runtime dependencies:

* Spark 3.5: `org.openprojectx.bigdata.test.core:extensions:<version>:runtime@jar`
* Spark 4.1: `org.openprojectx.bigdata.test.core:extensions-spark-4:<version>:runtime@jar`

The normal `extensions` artifact does not package Spark, Hadoop, Kafka, Trino JDBC, Log4j, or SLF4J implementations; those dependencies are supplied by the plugin-owned runtime configuration when needed. Both shaded artifacts package extension runtime dependencies. Jackson is aligned with the selected Spark line and relocated inside the shaded artifact to avoid collisions with Jackson versions used by the application or Gradle build logic. Spark 3 and Spark 4 are never packaged together.
...
|Use the `bigDataTestExtensionRuntime` configuration, or set `extensionRuntime.useShadedArtifact.set(false)` and configure `hadoopVersion`, `sparkVersion`, `scalaBinaryVersion`, `icebergVersion`, `confluentVersion`, `avroVersion`, `awsSdkVersion`, `trinoVersion`, and `lz4Version`.
