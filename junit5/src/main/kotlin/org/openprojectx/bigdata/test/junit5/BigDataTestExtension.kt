        ...
        if (icebergRestCatalog) {
            ...
            builder.withIcebergRestCatalog(
                IcebergRestCatalogOptions(
                    ...
                    catalogName = config.icebergRestCatalog.catalogName ?: defaults.catalogName,
                    warehouse = config.icebergRestCatalog.warehouse ?: defaults.warehouse,
                    realm = config.icebergRestCatalog.realm ?: defaults.realm,
                    clientId = config.icebergRestCatalog.clientId ?: defaults.clientId,
                    clientSecret = config.icebergRestCatalog.clientSecret ?: defaults.clientSecret,
                    scope = config.icebergRestCatalog.scope ?: defaults.scope,
                    s3RoleArn = config.icebergRestCatalog.s3RoleArn,
                    s3ExternalId = config.icebergRestCatalog.s3ExternalId,
                    startupTimeoutSeconds = config.icebergRestCatalog.startupTimeoutSeconds?.toLong()
